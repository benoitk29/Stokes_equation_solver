...
for nu = viscosites
    ...
    for l=1:Nbtri
        ...
        for i=1:6
            ...
            for j=1:6
                J=Numtri(l,j);
                MM(I,J)=MM(I,J)+Mel(i,j); % A priori pas utile ?
    ...
    %%affiche_bords(Coorneu,Numtri,Refneu);% à supprimer
    ...
    affiche_ordre2(U1, Numtri, Coorneu,sprintf('U1 - %s pour nu =  %.2f', nom_maillage,nu));
    xlabel('x');
    ylabel('y');
    affiche_ordre2(U2, Numtri, Coorneu,sprintf('U2 - %s pour nu =  %.2f', nom_maillage,nu));
    xlabel('x');
    ylabel('y');
    ...
    affiche_ordre1(P, NumtriDecale, Coorneu(numsommets,:),sprintf('pression - %s pour nu =  %.2f', nom_maillage,nu));
    xlabel('x');
    ylabel('y');
    figure(5) %Toutes les figures de ce type sont identiques, donc en trace qu'une et on mets dans la boucle afin de pouvoir en même
    %temps les autres figures avec des flèches en mettant des breakpoints
    %en fin de boucle
    quiver(Coorneu(:,1),Coorneu(:,2),U1,U2);
    title(sprintf('Vitesse pour nu=%.2f', nu) )
    xlabel('x');
    ylabel('y');
end
