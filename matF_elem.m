...
invB = inv(B);
invBT=invB';
L2 = invBT(2,:);% Ici, nous prenons seulement la deuxième ligne de l'inverse transposée de Bl
...
for q = 1:size(S_hat, 1)
    ...
    w_P1= [1 - S_hat(q, 1) - S_hat(q, 2);S_hat(q, 1);S_hat(q, 2)];
    ...
    wb = w_P1*L2 ;
    % Contribution à la matrice Fel
    Fel = Fel + poids *(-(wb * gradN_hat))'* detB ;
end
